...
import { AddMoreUnitsComponent } from '../../enrolment/new-student/dialog/add-more-units/add-more-units.component'
...
export class NewStudentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // console.log('anything', this.courseIntakeID)
    //New Student
    this.stepLabel = 1
    ...
    if (!window.localStorage.getItem('agents')) {
      // console.log('value is not passed')
      this.apiService.getAPI(`getagent`).subscribe((data) => {
        // console.log('postcodes',data)
        this.agents = data['data']
      ...
      // console.log('value passed',this.agents)
    }
    // this.apiService.getAPI('getagent').subscribe((data) => {
    //   //  console.log(data);
    ...
    if (!window.localStorage.getItem('trainingContract')) {
      this.apiService.getAPI(`gettrainingcontract`).subscribe((data) => {
        // console.log('trainingcontract',data)
  ...
  getCourses() {
    let courseintaledateid = this.courseIntakeID
    this.apiService.getAPI(`getcourseintakedate?id=${courseintaledateid}`).subscribe((data) => {
      //alert('test');
      //console.log(data);
      this.courses = data['data'];
      this.HFormGroup4.patchValue({
  ...
  onEnrolemntSubmit(stepper: MatStepper) {
    ...
    if (this.HFormGroup4.valid) {
      ...
      this.apiService.postAPI('addstudentenrolment', this.HFormGroup4.value).subscribe((data) => {
