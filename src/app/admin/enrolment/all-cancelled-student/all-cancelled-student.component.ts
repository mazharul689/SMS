...
@Component({
  selector: 'app-all-cancelled-student',
  templateUrl: './all-cancelled-student.component.html',
  styleUrls: ['./all-cancelled-student.component.sass'],
  providers: [
    {
      provide: DateAdapter,
...
export class AllCancelledStudentComponent implements OnInit {
mode = new FormControl("side");
  ...
  ngOnInit() {
    ...
    this.bulkApplicationStatusFilter.patchValue(5)
  ...
  getStudents() {
    this.apiService.getAPI('getstudentcancelled?applicationstatusid=5').subscribe((data) => {
      //console.log(data);
  ...
  search(cid: any, aid: any, asid: any, clid: any, uid: any, name: any) {
    ...
    if (queryParams.length > 0) {
      ...
      this.apiService.getAPI(`getstudentcancelled?${queryString}`).subscribe((data) => {
        // console.log(data);
  ...
  searchByAgent() {
    if (this.agentFilter.value > 0) {
      let id = this.agentId;
      this.apiService.getAPI(`getstudentcancelled?agentid=${id}`).subscribe((data) => {
