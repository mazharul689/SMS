...
export class AllStudentComponent implements OnInit {
  ...
  students
  displayedColumns: string[] = ['clientId', 'fullName', 'courseName', 'className', 'enrolDate', 'enroledStatus', 'expectedCompletionDate', 'actions']
  ...
  ngOnInit() {
    ...
    this.expectedCompletionDateFilter.valueChanges.subscribe(expectedCompletionDate => {
  ...
  createFilter(): (data: any, filter: string) => boolean {
    let filterFunction = function (data, filter): boolean {
      ...
      return data.clientid.toLowerCase().toString().indexOf(searchTerms.clientid.toLowerCase()) !== -1
        ...
        && (data.commencementdate || '').toLowerCase().indexOf(searchTerms.commencementdate.toLowerCase()) !== -1
        && (data.applicationstatusname || '').toLowerCase().indexOf(searchTerms.applicationstatusname.toLowerCase()) !== -1
        && (data.expectedcompletiondate || '').toLowerCase().indexOf(searchTerms.expectedcompletiondate.toLowerCase()) !== -1;
    }
  ...
  editStudent(row) {
    if (row.studentenrolmentid == null) {
      var step = 'P'
      this.router.navigate([`/admin/enrolment/edit-student/${step}/${row.studentid}`]);
    }
    else {
      var step = 'S'
      this.router.navigate([`/admin/enrolment/edit-student/${step}/${row.studentenrolmentid}`]);
    }
  }
  editEnrCourse(id) {
    var step = 'C'
  ...
  deleteStudent(enrolid, sid) {
    let step = 'E'
    if (enrolid) {
      this.router.navigate([`/admin/enrolment/unenroll-student/${step}/${enrolid}`]);
    }
    else {
      step = 'S'
      this.router.navigate([`/admin/enrolment/unenroll-student/${step}/${sid}`]);
    }
  }
  paymentSchedule(id) {
  ...
  downloadOfferLetter(id) {
    window.open(`https://api.wonderit.com.au:8000/report/offer_letter?inst_id=${this.userInfo.college_id}&sid=${id}`)
  }
  assignUnits(id) {
